...
let filterPlayer = [];
filterPlayer = requestrecords.filter((item) => {
    return item.receiving_user == loginuserid && item.request_status=="pending"
});


for (const userrequested of filterPlayer) {
    const { requested_user } = userrequested
    const userDetails = user_record.find((usersdata) => requested_user == usersdata.user_id);
...
function playerData(array = []) {
    ...
    array.forEach((item) => {
        ...
        anchor.setAttribute("href", "../../pages/player/playerprofile.html?player_id=" + item.requested_userDetails.user_id)
...
declinebtn.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        let reqIndex2 = requestrecords.findIndex((request2) => {
            return request2.requested_user == req_userId && request2.receiving_user==loginuserid
        })
